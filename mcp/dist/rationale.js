 ...
 */
export function explainQuote(q, c) {
    const out = [];
    const b = q.breakdown;
    if (!c.exists) {
    ...
    out.push(`Score ${b.score} of 1000, from three components: identity ${b.identitySubtotal}, ` +
        `credit history ${b.historySubtotal}, verified assets ${b.assetSubtotal}.`);
    out.push(`Identity: verification tier ${c.tier} contributes ${b.tierPoints}, sub-tier ${c.subTier} ` +
    ...
    if (b.repaymentPoints === 0n && b.defaultPenalty === 0n) {
        ...
        out.push(`Credit history: ${b.repaymentPoints} points from qualifying repayments and ` +
    ...
    out.push(`Approved. Credit line ${amount(q.creditLine)} aUSDC, of which ${amount(q.alreadyDrawn)} is ` +
        `already drawn; up to ${amount(q.maxDrawNow)} is available now.`);
    out.push(`Collateral required ${amount(q.collateralRequired)} aUSDC at ${pct(q.aprBps)} APR — ` +
        `less than the principal, which is the whole point: the shortfall is covered by standing, ` +
        `not by assets.`);
    out.push("The credit line is held against the identity behind the credential, not this wallet. " +
        "A second wallet under the same identity draws on the same line.");
    return out;
}
export function explainCredential(c, h, wallets) {
    if (!c.exists)
        return ["No Cleanverse A-Pass is bound to this address."];
    const out = [
    ...
    if (c.previousKycHash !== `0x${"0".repeat(64)}`) {
        out.push(`This credential supersedes ${c.previousKycHash}. The protocol unions the two, so the ` +
            `history below survived the re-verification.`);
    }
    out.push(`History with this protocol: ${h.loansOriginated} originated, ${h.loansRepaid} repaid, ` +
...
export function explainTerms(t) {
    if (!t.eligible)
        return "Below the minimum standing; no terms are offered at any collateral level.";
